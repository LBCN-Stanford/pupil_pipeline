...
def plot_raw(raw, name='raw_plot', title='Raw Data Plot', out_dir='', base_name='', **params):
    ...
    plt.savefig(make_path(name, '.png', out_dir=out_dir, base_name=base_name, **params))
    plt.cla()
    plt.clf()
    plt.close()
...
def check_outliers(raw, impossible_upper=5, impossible_lower=1.5, out_dir='', base_name='', **params):
    ...
    raw.loc[(raw.Pupil < impossible_lower) | (raw.Pupil > impossible_upper), 'Pupil'] = np.nan
...
def calculate_stats(raw, fname='descriptive_stats',plot=True, out_dir='', base_name='', **params):
    ...
    if plot:
        ax = raw.plot(x='Time', y='Pupil', kind='scatter', title='Raw Data (Outliers Removed)')
        ...
        plt.clf()
        plt.close()
    stats.to_csv(make_path(fname,'.csv', out_dir=out_dir, base_name=base_name))
