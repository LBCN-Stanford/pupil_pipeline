...
def read_events(fname, p_first):
    ...
    try:
        ...
        print('Make sure your events file has a struct called events, and a float called first!')
        sys.exit()
...
def get_nsamples(sample_rate=250, epoch_time=200, back_time=60, **params):
    ...
    '''
    #TODO Replace
    return (int(sample_rate * back_time / 1000), int(sample_rate * epoch_time / 1000))
...
class Epoched:
    ...
    def __init__(self, n_categs, n_samples, n_trials):
        ...
        self.matrix[:] = np.nan
        self.n_categs, self.n_samples, self.n_backsamples = n_categs, n_samples[1], n_samples[0]
...
def get_baseline(pupil_data, onset, baseline_type='no', sample_rate=250, bl_events=None):
    if baseline_type == 'no':
        ...
        if len(difference) == 0: return np.nan
...
def get_baseline_events(pupil_events, category):
    onsets = list(map(lambda x: get_nearest_ind(pupil_events, x), category.start))
...
def epoch(pupil_data, events_path, sample_rate=250, epoch_time=200,
    ...
    if type(baseline_type) == tuple:
        bl_events = get_baseline_events(pupil_events, categories[baseline_type[1]])


    for c, category in enumerate(categories):
        epoched.names.append(category.name)
        # +1 to exclude the content sample, which is out of sync.
        onsets = list(map(lambda x: get_nearest_ind(pupil_events, x) + 1, category.start))
        rejected = 0
        for t, onset in enumerate(onsets):
            # Note the -1 to avoid the content sample
            pre = pupil_data.Pupil.iloc[onset-samples_per_epoch[0] - 1:onset - 1].values
            post = pupil_data.Pupil.iloc[onset:onset + samples_per_epoch[1]].values
            baseline = get_baseline(pupil_data, onset, baseline_type, sample_rate, bl_events)
            trial = np.concatenate((pre, post)) - baseline

            if np.isnan(np.sum(trial)): rejected += 1 # Checks if there is a nan
