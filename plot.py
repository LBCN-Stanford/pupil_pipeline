...
def plot_conds(epoched, conds_to_plot='all', plot_colors=def_cols,
	...
	if conds_to_plot =='all': conds_to_plot=[i for i in range(epoched.n_categs)]
	assert len(plot_colors) >= len(conds_to_plot), 'Require more colors than plotted conditions'
	assert len(plot_style) > len(conds_to_plot), 'Require more styles than plotted conditions'
	x = [1000 * i / sample_rate - back_time for i in range(epoched.total_samples)]
